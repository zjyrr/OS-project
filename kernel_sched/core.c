...
//----------------------------------------------------------------
	case SCHED_WRR:
		...
		p->wrr.weight = prio;
		p->wrr.p = p;
		p->wrr.time_slice = TIME_SLICE * prio;
		p->wrr_group = current_uid();
		printk("PROCESS INSERTED WITH ID %d \n" , current_uid());
		break;
//----------------------------------------------------------------
	}
...
static int __sched_setscheduler(struct task_struct *p, int policy,const struct sched_param *param, bool user) //revise add_:
{
